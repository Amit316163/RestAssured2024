...
//	public static final String AUTHORIZATION = "Bearer 2043b7426a320fb4bbe283fef008aabf2ea1163e989c984c9b02facbe7f29378";
	...
	ConfigurationManger config=new ConfigurationManger();
...
//	}
	...
	private RequestSpecification createRequestSpec(Map<String, String> headers, Map<String, String> params,boolean includeToken) {
		specBuilder.setBaseUri(baseURI);
		if(includeToken) {
			addAuthorization();
			}
	...
	// Post-given data --URI, Auth,contentType and Body
	private RequestSpecification createRequestSpec(String contentType, Object payload, Map<String, String> headers,boolean includeToken) {
		specBuilder.setBaseUri(baseURI);
		if(includeToken) {
			addAuthorization();
			}
	...
	public Response doGET(String basePath,Map<String,String> headers,boolean includeAuth,boolean log) {
		...
		if(log) {
			return RestAssured.given(createRequestSpec(headers,includeAuth)).log().all()				
					.when().get(basePath);
		}
		return RestAssured.given(createRequestSpec(headers,includeAuth))				
				.when().get(basePath);
		
	}
	
	//GET -- URI,Auth and Headers and Params
public Response doGET(String basePath,Map<String,String> headers,Map<String,String> params,boolean includeAuth,boolean log) {
		
		if(log) {
			return RestAssured.given(createRequestSpec(headers, params,includeAuth)).log().all()				
					.when().get(basePath);
		}
		return RestAssured.given(createRequestSpec(headers, params,includeAuth))				
				.when().get(basePath);
...
public Response doPost(String basePath,String contentType,Object payload,boolean includeAuth,boolean log) {
	...
	if(log) {
		return RestAssured.given(createRequestSpec(contentType, payload,includeAuth)).log().all()				
				.when().post(basePath);
	}
	return RestAssured.given(createRequestSpec(contentType, payload,includeAuth))				
			.when().post(basePath);
	
}

//POST -- URI,Auth ,contentType and body and Headers
public Response doPost(String basePath,String contentType,Object payload,Map<String,String> headers,boolean includeAuth,boolean log) {
	
	if(log) {
		return RestAssured.given(createRequestSpec(contentType, payload, headers,includeAuth)).log().all()				
				.when().post(basePath);
	}
	return RestAssured.given(createRequestSpec(contentType, payload, headers,includeAuth))				
			.when().post(basePath);
...
public Response doPut(String basePath,String contentType,Object payload,Map<String,String> headers,boolean includeAuth,boolean log) {
...
public Response doPatch(String basePath,String contentType,Object payload,Map<String,String> headers,boolean includeAuth,boolean log) {
...
public Response doDelete(String basePath,Map<String,String> headers,Map<String,String> params,boolean includeAuth,boolean log) {
